...
public class RCaNOutputParser {

    public static String decodeParser(RCaller caller, RCaNScript rCaNScript) {
        ROutputParser parser = caller.getParser();
        String result;
        if (rCaNScript.isTable()) {
            result =  decodeArray(parser, rCaNScript);
        } else {
            result = decodeString(parser);
        }
        return result;
    }
    // ------------------------------------------------------------------------
    static String decodeArray(ROutputParser parser, RCaNScript rCaNScript) {
        ...
        sb.append(rCaNScript.getTextMenu());
        sb.append("\n\n");
    ...
    // ------------------------------------------------------------------------
    static String decodeString(ROutputParser parser) {
        StringBuilder sb = new StringBuilder("");
        List<String> na = parser.getNames();
        for (String n : na) {
            sb.append("\n\n");
