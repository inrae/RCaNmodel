...
ggResult <- function(mcmc_res, myCaNmod, param, colour = "blue") {
  ...
  quantiles<-do.call('rbind.data.frame',lapply(param,function(p){
    ...
    quantiles <-
      data.frame(t(apply(mat_res[, columns], 2, quantile, probs = c(.025, .5, .975))),year=myCaNmod$series$Year,series=as.character(p))
  }))
  names(quantiles)[1:3] = c("q2.5", "q50", "q97.5")
  ggplot()+geom_line(data=quantiles,aes_string(x="year",y="q50",col="series")) + geom_ribbon(data=quantiles,aes_string(x="year",ymin = "q2.5", ymax = "q97.5",fill="series"),
                                                                                           alpha = .33) + ylab(param)
}
