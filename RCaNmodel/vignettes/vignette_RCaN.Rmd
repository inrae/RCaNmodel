---
title: 'An application of CaN modelling to the Barents  Sea trophic network'
date: "5/2/2021"
output: 
...
vignette: >
  %\VignetteIndexEntry{An application of CaN modelling to the Barents  Sea trophic network}
...
# Introduction
This vignette was initially published as a supplementary material of a scientific article.
...
The RCaN-file, constructed with RCaNconstructor, is attached to this supplementary information. 
...
system.file("extdata", "BarentsSeaDummy.xlsx", package = "RCaNmodel")


NAMEFILE <- system.file("extdata",
                        "BarentsSeaDummy.xlsx",
                        package = "RCaNmodel")
```
...
if (requireNamespace("kableExtra" , quietly = TRUE)) { 
  kable(POLYTOPE$constraints[,1:5],
        caption='Constraints defined by user') %>%
    kableExtra::kable_styling(latex_options = c("striped", "scale_down"),
                              font_size = 8)
} else {
...
```{r label=sample}
if (file.exists(system.file("extdata", "sampleCaN.rdata", package = "RCaNmodel"))){
  system.file("extdata", "sampleCaN.rdata", package = "RCaNmodel")
} else {
